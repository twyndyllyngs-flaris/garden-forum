...
function Forum () {
  ...
  const openForumDialog = async (forum: any) => {
    ...
    setForumDialog(true)
    navigate(
      `/forum/${forum.forum_id}?comment_id=${searchParams.get('comment_id')}`
    )
  }
              ...
              >
                <CardHeader className='relative'>
                  ...
                  <div className='w-full flex items-center gap-3 cursor-pointer'>
                    ...
                    >
                      <AvatarImage src={forum.profiles?.profile_link} />
                      <AvatarFallback className='bg-red-200 text-[.7rem] font-semibold text-gray-800'>
                        {userProfile
                          ? getInitials(
                              forum.profiles ? forum.profiles?.first_name : '',
                              forum.profiles ? forum.profiles?.last_name : ''
                            )
                          : 'USER'}
                      </AvatarFallback>
                    </Avatar>
      ...
      <AlertDialog open={isCreateSpaceOpen} onOpenChange={setCreateSpace}>
        <AlertDialogContent>
          <form onSubmit={handlePost}>
            <AlertDialogHeader>
              <AlertDialogTitle className='flex justify-center mb-2'>
                Create Post
              </AlertDialogTitle>
            ...
            <AlertDialogFooter className='mt-4'>
              <AlertDialogCancel onClick={closeCreateSpace}>
                Cancel
              </AlertDialogCancel>
              <Button type='submit' disabled={loading}>
                ...
                {/* Change text to 'Posting...' when loading */}
              </Button>
            </AlertDialogFooter>
          </form>
        </AlertDialogContent>
          ...
          <div className='w-full h-full max-h-full overflow-auto flex rounded-md'>
            ...
            <div className='w-[30%] h-full max-h-full overflow-auto p-6 relative'>
              <div className='flex gap-2 items-center'>
                ...
                >
                  <AvatarImage src={openedForumData?.profiles?.profile_link} />
                  <AvatarFallback className='bg-red-200 text-[.7rem] font-semibold text-gray-800'>
                    {getInitials(
                      openedForumData?.profiles
                  ...
                  {openedForumComments?.map((comment: any, index: number) => (
                    <div
                      ...
                      className={`flex flex-col ${
                        index == 0 &&
                        searchParams.get('comment_id') === comment.comment_id
                    ...
                    >
                      <div className='flex gap-4 qwe rrrrrrrrrrrrw-full'>
                        ...
                        >
                          <AvatarImage
                            src={
                              forums.find(forum => forum.uid === comment.uid)
              ...
              <div className='w-[390px] h-24 sticky -bottom-6 bg-white -mb-6 -ml-6 -mr-6 px-3 pb-3 box-content'>
                <Textarea
                  ...
                  placeholder={
                    loggedInUser
                      ? 'Comment as ' + loggedInUser?.user_metadata.displayName
                      : 'Login to comment'
                  }
                  required
                  value={newComment}
