...
function Navbar () {
  ...
  const getUserProfileById = async (uid: string) => {
    const { data, error } = await supabase
      .from('profiles')
      .select('profile_link')
      .eq('uid', uid)

    return data ? data[0].profile_link : ''
  }

  useEffect(() => {
    if (notifications.length === 0) return // No notifications, no need to fetch profiles

    const fetchProfiles = async () => {
      try {
        // Extract unique sender IDs from notifications
        ...
        if (error) {
          ...
          // Update the userProfiles state with the fetched profiles
  ...
  const navigateNotifaction = async (notification: NotificationPayload) => {
    ...
    if (notification.type === 'System1') {
      ...
      navigate(
        `/forum/${notification.forum_id}?comment_id=${notification.comment_id}`
      )
    }

    if (!notification.read) handleReadNotif(notification.notification_id)
  }
      ...
      <div className='container mx-auto flex justify-between items-center'>
        ...
        <div className='flex space-x-4 text-md text-gray-500'>
          <Link
            ...
            className={`hover:text-gray-700 px-3 py-2 rounded-md font-medium ${
              location.pathname === '/guides' ? 'text-gray-700' : ''
            }`}
          >
            Guides
          </Link>
          <Link
            to='/forum'
            className={`hover:text-gray-700 px-3 py-2 rounded-md font-medium ${
              location.pathname === '/forum' ? 'text-gray-700' : ''
            }`}
          >
            Forum
          </Link>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className='w-fit max-w-[300px] max-h-[600px] overflow-auto relative pt-0'>
              <DropdownMenuLabel
                className='sticky top-0 bg-white z-10 border-b-[1px] border-gray-200 mb-1'
                onClick={test}
              >
                {notifications.length === 0
                  ? 'Your notifaction will show here'
                  : 'Notifications'}
              </DropdownMenuLabel>
              ...
              <DropdownMenuGroup className='flex flex-col gap-2'>
                {notifications.map(notification => (
                  <DropdownMenuItem
                    ...
                    } cursor-pointer flex flex-col items-start focus:bg-gray-200`}
                  >
                            ...
                            userProfiles
                              ? userProfiles.find(
                                  (profile: { uid: string | null }) =>
                                    profile.uid === notification.sender_id
                                ).profile_link
                              : ''
                          }
                        />
                        <AvatarFallback className='bg-red-200 text-[.7rem] font-semibold text-gray-800'>
                          {getInitials(
                            userProfiles
                              ? userProfiles.find(
                                  (profile: { uid: string | null }) =>
        ...
        {loading ? (
          <div className='flex justify-center items-center gap-3 cursor-pointer'>
            ...
            <Avatar>
              <AvatarImage src={userProfile ? userProfile.profile_link : ''} />
              <AvatarFallback className='bg-red-200 text-[.7rem] font-semibold text-gray-800'>
                {getInitials(
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <div className='flex justify-center items-center gap-3 cursor-pointer'>
                ...
                <Avatar>
                  <AvatarImage
                    src={userProfile ? userProfile.profile_link : ''}
                  />
                  <AvatarFallback className='bg-red-200 text-[.7rem] font-semibold text-gray-800'>
                    {getInitials(
                      userProfile ? userProfile.first_name : '',
            ...
            <DropdownMenuContent className='w-56'>
              <DropdownMenuLabel onClick={test}>My Account</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuGroup>
                <DropdownMenuItem
